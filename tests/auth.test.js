...
utils.testauth("findUserHandler works correctly", async () => {
...
utils.testauth("getPermissionsHandler fails correctly without authentication", async () => {
    let req = new Request("http://localhost");
...
utils.testauth("setPermissionsHandler works correctly", async () => {
    {
        let req = new Request("http://localhost", {
            method: "POST",
    ...
    // Breaks correctly if project doesn't exist.
    {
        ...
        let nb = [];
        await utils.expectError(auth.setPermissionsHandler(req, nb), "does not exist");
    }
